...
export default function TicketSuccessPage() {
  ...
  const handleDownload = useCallback(async () => {
    ...
    setDownloading(true);
    try {
      const res = await fetch(`/api/ticket/${ticket.id}/pdf`);
        ...
        <div className="bg-surface/80 backdrop-blur-md border border-border rounded-2xl overflow-hidden">
          ...
          <div className="px-6 pb-6">
            <button
              ...
              className="flex items-center justify-center gap-2 w-full bg-accent hover:bg-accent-dark disabled:opacity-70 disabled:cursor-wait text-black font-semibold py-3 rounded-xl transition-all"
            >
              {downloading ? (
                <>
